"""Tests for the simulation orchestration in the analytics service."""
...
def test_product_simulation_default_set() -> None:
    _, factory = make_business_db()
    with session_scope(factory) as session:
        ids = _seed(session)
        product, result = AnalyticsService(session).product_simulation(
        ...
        assert result.best_scenario in labels


@pytest.mark.integration
def test_product_simulation_custom_scenarios() -> None:
    _, factory = make_business_db()
    with session_scope(factory) as session:
        ids = _seed(session)
        specs = [
            ScenarioSpec(ScenarioType.BASELINE, label="Baseline"),
            ScenarioSpec(ScenarioType.FIXED_PRICE, label="$3.50", price=3.5),
        ]
        _, result = AnalyticsService(session).product_simulation(
            ids["p1"], objective=Objective.MAXIMIZE_GROSS_PROFIT, scenarios=specs
        )
        assert {s.label for s in result.scenarios} == {"Baseline", "$3.50"}


@pytest.mark.integration
def test_dataset_simulation_runs() -> None:
    _, factory = make_business_db()
    with session_scope(factory) as session:
        _seed(session)
        result = AnalyticsService(session).dataset_simulation(objective=Objective.MAXIMIZE_REVENUE)
...
def test_missing_product_raises_not_found() -> None:
    ...
    with session_scope(factory) as session:
        ...
        with pytest.raises(NotFoundError):
            AnalyticsService(session).product_simulation(
